    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value.length > 30) {
          ...
          draft.username.message = "Username canoot exceed 30 characters"
        }
      ...
      case "usernameAfterDelay":
       if(draft.username.value.length < 3){
        draft.username.hasErrors = true
         draft.username.message = "Username has to be 3 characters"
       }
      ...
      case "emailUniqueResults":
        ...
        return
      case "passwordImmediatelly":
        draft.password.hasErrors = false
        draft.password.value = action.value
        if(draft.password.value.length > 50){
          draft.password.hasErrors = false
          draft.password.message = "The password should not exceed 50 characters"
        }
        return
      case "passwordAfterDelay":
        ...
        if(draft.password.length < 12){
          ...
          draft.password.message = "The password should be more then 12 characters"
        }
        return
      case "submitForm":
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post('/doesEmailExist', { email: state.username.value }, { cancelToken: ourRequest.token })
  ...
  useEffect(() => {
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post('/register', { email: state.username.value, email: state.email.value, password: state.password.value }, { cancelToken: ourRequest.token })
